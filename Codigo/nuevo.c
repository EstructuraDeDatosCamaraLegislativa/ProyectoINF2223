...
void agregarVoto(struct VotacionParlamentarios **votacionLista, int id, int voto) {
...
void realizarVotacionCamara(struct CamaraLegislativa *camara, char *resultado, int esDiputados) {
    ...
    if (esDiputados) {
        ingresarVotos(&(camara->proyecto->camaraDiputados)); // Ingresar votos para Diputados
        ...
        ingresarVotos(&(camara->proyecto->camaraSenado)); // Ingresar votos para Senado
...
void manejarDesacuerdo(struct ComisionMixta *comision) {
    ...
    ingresarVotos(&comision->proyecto->comisionMixta);
