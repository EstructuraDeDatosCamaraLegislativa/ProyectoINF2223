...
/* Función para inicializar las cámaras legislativas */
void inicializarCamaras(struct ProcesoLegislativo *proceso) {
    for (int i = 0; i < 2; i++) {
        proceso->camaras[i] = (struct CamaraLegislativa *)malloc(sizeof(struct CamaraLegislativa));
        
        // Verificar si la asignación de memoria fue exitosa
        if (proceso->camaras[i] == NULL) {
            printf("Error al asignar memoria para la cámara %d. No se podrá usar esta cámara.\n", i + 1);
            proceso->camaras[i] = NULL;
        } else {
            // Asignar el tipo de cámara según la posición en el array
            if (i == 0) {
                strcpy(proceso->camaras[i]->tipoCamara, "Diputados");
            } else {
                strcpy(proceso->camaras[i]->tipoCamara, "Senado");
            }

            // Solicitar el nombre de la cámara
            printf("Ingrese el nombre de la cámara (%s): ", proceso->camaras[i]->tipoCamara);
            fgets(proceso->camaras[i]->nombreCamara, sizeof(proceso->camaras[i]->nombreCamara), stdin);
            proceso->camaras[i]->nombreCamara[strcspn(proceso->camaras[i]->nombreCamara, "\n")] = 0;

            proceso->camaras[i]->proyectoActual = NULL;
            proceso->camaras[i]->parlamentarios = NULL;
        }
    }
    printf("Configuración de cámaras completada.\n");
}
...
void procesarOpcion(int opcion, struct ProcesoLegislativo *proceso) {
    if (opcion == 2) {
        if (proceso->camaras[0] != NULL && proceso->camaras[1] != NULL) {
            printf("Las cámaras ya han sido configuradas.\n");
        } else {
            inicializarCamaras(proceso);
        }
    } else if (proceso->camaras[0] != NULL && proceso->camaras[1] != NULL) { // Verificar que las cámaras estén configuradas para otras opciones
        if (opcion == 1) {
        ...
        printf("Debe configurar las cámaras antes de realizar otras operaciones.\n");
    }
}
...
int main() {
    ...
    iniciarSistema(PL); // Iniciar el menú interactivo
    return 0;
}
