...
// Función para agregar un voto a la lista
void agregarVoto(struct VotacionParlamentarios **votacionLista, int id, int voto, int proyectoID) {
    struct VotacionParlamentarios *nuevoVoto = (struct VotacionParlamentarios *)malloc(sizeof(struct VotacionParlamentarios));
    nuevoVoto->parlamentarioID = id;
    nuevoVoto->voto = voto;
    nuevoVoto->proyectoID = proyectoID;
    nuevoVoto->siguiente = *votacionLista;
    *votacionLista = nuevoVoto;
}
...
void realizarVotacionCamara(struct CamaraLegislativa *camara, char *resultado, int esDiputados) {
    ...
    if (esDiputados) {
        ingresarVotos(&(camara->proyecto->camaraDiputados), camara->proyecto->ID); // Ingresar votos para Diputados
        strcpy(resultado, resultadoVotacion(camara->proyecto->camaraDiputados)); // Calcular el resultado
        mostrarResultadoCamara("Diputados", resultado); // Mostrar el resultado
    } else {
        ingresarVotos(&(camara->proyecto->camaraSenado), camara->proyecto->ID); // Ingresar votos para Senado
        strcpy(resultado, resultadoVotacion(camara->proyecto->camaraSenado)); // Calcular el resultado
...
void crearTablaHash(struct HashTable *tabla, struct ProyectoLey **arreglo, int tamano) {
    ...
    for (i = 0; i < tamano; i++) {
        // Asegurarnos de que el puntero no sea NULL antes de acceder a su campo ID
        if (arreglo[i] == NULL) {
            continue;
        }
        ...
        printf("Proyecto con ID %d insertado en la posición %d\n", arreglo[i]->ID, j);
    }
}
...
struct ProyectoLey* buscarProyectoPorID(struct HashTable *tabla, int ID) {
    ...
    while (tabla->proyectos[j] != NULL) {
        if (tabla->proyectos[j]->ID == ID) {
            printf("Proyecto con ID %d encontrado en la posición %d\n", ID, j);  // Mensaje de depuración
