...
// Función para buscar un proyecto en el arreglo de proyectos
struct ProyectoLey* buscarProyectoEnArreglo(struct ProyectoLey **arreglo, int tamano, int ID) {
    int i;
    for (i = 0; i < tamano; i++) {
        if (arreglo[i] != NULL && arreglo[i]->ID == ID) {
            return arreglo[i];  // Proyecto encontrado
        }
    }
...
void arbolAArregloDesordenado(struct NodoArbol* nodo, struct ProyectoLey **arreglo, int *tamano) {
    int indice = 0;
    recorridoPreorden(nodo, arreglo, &indice);  // Llenar el arreglo usando recorrido preorden
    *tamano = indice;  // El tamaño del arreglo es el número de proyectos en el árbol
}

// Función para buscar un proyecto en el arreglo desordenado (sin tabla hash)
struct ProyectoLey* buscarProyectoPorID(struct NodoArbol *nodo, int ID) {
    static struct ProyectoLey *arreglo[TAM];  // Arreglo estático para almacenar proyectos
    static int tamano = 0;  // Variable estática para almacenar el tamaño del arreglo

    // Llenar el arreglo con los proyectos del ABB si aún no se ha hecho
    if (tamano == 0) {
        arbolAArregloDesordenado(nodo, arreglo, &tamano);
    }
...
void modificarProyecto(struct ProcesoLegislativo *proceso) {
    ...
    proyecto = buscarProyectoPorID(proceso->Proyectos, ID); // Asumiendo que 'tablaProyectos' es la tabla hash dentro de 'proceso'
...
struct NodoArbol* eliminarNodoProyecto(struct NodoArbol* nodo, int ID, int *eliminado) {
    ...
    if (ID < nodo->proyecto->ID) {
        ...
        *eliminado = 1; // Marca que se encontró y se debe eliminar
        ...
        temp = encontrarMinimo(nodo->der); // Encuentra el nodo mínimo en el subárbol derecho
    ...
    return nodo; // Retorna la raíz del árbol (o subárbol) modificado
}
